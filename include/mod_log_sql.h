...
typedef struct {
    ...
    int createtables;		/* Should we create the tables ? */
    apr_table_t *parms;		/* DB connection parameters */
#if APR_HAS_THREADS
    apr_thread_mutex_t *mutex;	/* Thread locking, as DB handle cannot be shared */
#endif
} logsql_dbconnection;
...
typedef struct {
    ...
    const char *(*escape) (request_rec * r, const char *src, apr_pool_t * p, logsql_dbconnection * db);
    /* insert a SQL query statement */
    logsql_query_ret(*insert) (request_rec * r, logsql_dbconnection * db, const char *query);
