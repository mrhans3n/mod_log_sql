...
#define DEFAULT_PRESERVE_FILE		"/var/log/apache2/mod_log_sql-preserve.sql"
 ...
 * @note This is exported from the module
 */
LOGSQL_DECLARE(void) log_sql_register_alias(server_rec * s, apr_pool_t * p, char key, const char *alias)
{
    server_rec *ts;
...
LOGSQL_DECLARE(void) log_sql_register_field(apr_pool_t * p,
					    ...
					    const char *sql_field_name, logsql_field_datatype datatype, apr_size_t size)
{
    logsql_field *item;
...
LOGSQL_DECLARE(void) log_sql_register_finish(server_rec * s)
{
	...
	if (!item->func) {
	    log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "Could not find function %s for item %s", item->funcalias,
		      item->alias);
	}
    }
	...
	if (!cfg->parsed_log_format) {
	    cfg->parsed_log_format =
		apr_pcalloc(cfg->parsed_pool, cfg->transfer_log_format->nelts * sizeof(logsql_field *));
	}
...
LOGSQL_DECLARE(void) log_sql_register_driver(apr_pool_t * p, logsql_dbdriver * driver)
{
    const char *providername = apr_table_get(global_config.db.parms, "driver");

    if(strcmp(providername, driver->providername) == 0)
...
static logsql_opendb_ret log_sql_opendb_link(server_rec * s)
{
    ...
    if (global_config.db.parms && global_config.driver != NULL) {

	#if APR_HAS_THREADS
    	    apr_thread_mutex_lock(global_config.db.mutex);
	#endif
	logsql_opendb_ret retval = global_config.driver->connect(s, &global_config.db);
	#if APR_HAS_THREADS
    	    apr_thread_mutex_unlock(global_config.db.mutex);
	#endif

	return retval;
    }

    log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: insufficient configuration info to establish database link");
...
static void preserve_entry(request_rec * r, const char *query)
{
    ...
    result = apr_file_open(&fp, cls->preserve_file, APR_FOPEN_APPEND | APR_FOPEN_WRITE | APR_FOPEN_CREATE, APR_FPROT_OS_DEFAULT, r->pool);

    if (result != APR_SUCCESS) {
	log_error(APLOG_MARK, APLOG_DEBUG, result, r->server,
		  "attempted append of local preserve file '%s' but failed.", cls->preserve_file);
    } else {
	apr_file_lock(fp, APR_FLOCK_EXCLUSIVE);
	apr_file_printf(fp, "%s;\n", query);
	apr_file_unlock(fp);
	apr_file_close(fp);
	log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "mod_log_sql: entry preserved in %s", cls->preserve_file);
    }
}
...
static const char *set_global_flag_slot(cmd_parms * cmd, void *struct_ptr, int flag)
{
...
static const char *set_global_nmv_flag_slot(cmd_parms * cmd, void *struct_ptr, int flag)
{
    if (global_config.massvirtual) {
	return apr_psprintf(cmd->pool,
			    "mod_log_sql: do not set %s when LogSQLMassVirtualHosting(%d) is On.%d:%d",
			    cmd->cmd->name, global_config.massvirtual, (int) (long) &global_config,
			    (int) (long) struct_ptr);
    } else {
...
static const char *set_global_string_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
    void *ptr = &global_config;
    int offset = (int) (long) cmd->info;
...
static const char *set_server_string_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
...
static const char *set_server_file_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
    void *ptr = ap_get_module_config(cmd->server->module_config,
				     &log_sql_module);
    int offset = (int) (long) cmd->info;
    ...
    if (!path) {
	return apr_pstrcat(cmd->pool, "Invalid file path ", arg, NULL);
    }
...
static apr_array_header_t *create_logformat_default(apr_pool_t * p)
{
    ...
    logformat = apr_array_make(p, 32, sizeof(char *));
    addme = apr_array_push(logformat);
    *addme = "useragent";
    addme = apr_array_push(logformat);
    *addme = "request_args";
...
#endif
    addme = apr_array_push(logformat);
    *addme = "connection_status";
    addme = apr_array_push(logformat);
    *addme = "cookie";
    addme = apr_array_push(logformat);
    *addme = "local_address";
    addme = apr_array_push(logformat);
    *addme = "remote_address";
    addme = apr_array_push(logformat);
    *addme = "request_file";
    addme = apr_array_push(logformat);
    *addme = "request_protocol";
    addme = apr_array_push(logformat);
    *addme = "remote_host";
    addme = apr_array_push(logformat);
    *addme = "unique_id";
    ...
    *addme = "child_tid";
    addme = apr_array_push(logformat);
    *addme = "remote_logname";
    addme = apr_array_push(logformat);
    *addme = "machine_id";
    addme = apr_array_push(logformat);
    *addme = "request_method";
    addme = apr_array_push(logformat);
    *addme = "child_pid";
    addme = apr_array_push(logformat);
    *addme = "server_port";
    ...
    *addme = "request_line";
    addme = apr_array_push(logformat);
    *addme = "request_timestamp";
    addme = apr_array_push(logformat);
    *addme = "status";
    addme = apr_array_push(logformat);
    *addme = "request_duration";
    addme = apr_array_push(logformat);
    *addme = "request_time";
    addme = apr_array_push(logformat);
    *addme = "request_uri";
...
static const char *set_server_nmv_string_slot(cmd_parms * parms, void *struct_ptr, const char *arg)
{
    if (global_config.massvirtual)
	return apr_psprintf(parms->pool, "mod_log_sql: do not set %s when LogSQLMassVirtualHosting is On.",
			    parms->cmd->name);
    else
...
static const char *set_dbparam(cmd_parms * cmd, void *struct_ptr, const char *key, const char *val)
{
...
static const char *set_dbparam_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
...
static const char *set_log_sql_info(cmd_parms * cmd, void *dummy, const char *uristr)
{
    apr_uri_t uri;

    if (uristr) {
	if (apr_uri_parse(cmd->pool, uristr, &uri) == APR_SUCCESS) {
...
static const char *add_server_string_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
    char **addme;
    void *ptr = ap_get_module_config(cmd->server->module_config,
				     &log_sql_module);
...
static const char *set_logformat_slot(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
    const char *t;
    char t2[2] = { '\0', '\0' };
    for (t = arg; *t != '\0'; t++) {
...
static const char *set_register_field(cmd_parms * cmd, void *struct_ptr, const char *arg)
{
    ...
    } else if (strcasecmp("TIMESTAMP", datatype_s) == 0) {
	datatype = LOGSQL_DATATYPE_TIMESTAMP;
    } else {
    ...
    log_sql_register_field(cmd->pool, alias, funcalias, param, field, datatype, size);
 ...
 * that are defined in the array 'log_sql_module' (at EOF) *
 *------------------------------------------------------------*/
/* Closing sql link: child_exit(1.3), pool registration(2.0) */
...
#if APR_HAS_THREADS
    apr_thread_mutex_destroy(global_config.db.mutex);
#endif
    return APR_SUCCESS;
}
...
static void log_sql_child_init(apr_pool_t * p, server_rec * s)
{
    ...
    global_config.db.handle = NULL;

#if APR_HAS_THREADS
    apr_thread_mutex_create(&global_config.db.mutex, APR_THREAD_MUTEX_UNNESTED, s->process->pool);
#endif
    /* Register cleanup hook to close DDB connection (apache 2 doesn't have child_exit) */
    apr_pool_cleanup_register(p, NULL, log_sql_close_link, apr_pool_cleanup_null);
	...
	if (global_config.driver == NULL) {
	    log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: Driver module not loaded");
	} else {
	    log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: child spawned but unable to open database link");
	}
	break;
    case LOGSQL_OPENDB_SUCCESS:
	log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: log_sql_opendb_link successful");
	break;
    case LOGSQL_OPENDB_ALREADY:
	log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: log_sql_opendb_link already connected");
	break;
    case LOGSQL_OPENDB_PRESERVE:
	log_error(APLOG_MARK, APLOG_DEBUG, 0, s, "mod_log_sql: log_sql_opendb_link preservation forced");
	break;
    }
}
...
static int log_sql_post_config(apr_pool_t * p, apr_pool_t * plog, apr_pool_t * ptemp, server_rec * s)
{
	...
	if (result != APR_SUCCESS) {
	    log_error(APLOG_MARK, APLOG_DEBUG, result, s, "attempted open of showconfig file '%s' failed.", tempfile);
	...
	if (strcmp(cls->preserve_file, DEFAULT_PRESERVE_FILE) == 0)
...
#endif
    log_sql_register_function(p, "connection_status", extract_connection_status, LOGSQL_FUNCTION_REQ_FINAL);
    log_sql_register_function(p, "cookie", extract_cookie, LOGSQL_FUNCTION_REQ_FINAL);
    log_sql_register_function(p, "local_address", extract_local_address, LOGSQL_FUNCTION_REQ_FINAL);
    log_sql_register_function(p, "remote_address", extract_remote_address, LOGSQL_FUNCTION_REQ_FINAL);
    ...
    log_sql_register_function(p, "child_tid", extract_child_pid, LOGSQL_FUNCTION_REQ_ORIG);
    ...
    log_sql_register_function(p, "referer", extract_referer, LOGSQL_FUNCTION_REQ_ORIG);
    log_sql_register_function(p, "request_line", extract_request_line, LOGSQL_FUNCTION_REQ_ORIG);
    log_sql_register_function(p, "request_timestamp", extract_request_timestamp, LOGSQL_FUNCTION_REQ_FINAL);
...
#endif
    log_sql_register_alias(s, p, 'C', "connection_status");
    log_sql_register_alias(s, p, 'c', "cookie");
    log_sql_register_alias(s, p, 'D', "local_address");
    log_sql_register_alias(s, p, 'd', "remote_address");
    ...
    log_sql_register_alias(s, p, 'L', "child_tid");
    ...
    log_sql_register_alias(s, p, 'R', "referer");
    log_sql_register_alias(s, p, 'r', "request_line");
    log_sql_register_alias(s, p, 'S', "request_timestamp");
    ...
    log_sql_register_field(p, "request_timestamp",	"request_timestamp",	NULL,	"request_timestamp",	LOGSQL_DATATYPE_INT,		20);
...
static logsql_query_ret safe_sql_insert(request_rec * r, logsql_tabletype table_type, const char *table_name,
					const char *query)
{

    logsql_query_ret result;
    logsql_table_ret table_result;
	...
	if (global_config.db.createtables) {
	    log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "table doesn't exist...creating now");

	    table_result = global_config.driver->create_table(r, &global_config.db, table_type, table_name);

	    if (table_result != LOGSQL_TABLE_SUCCESS) {
		log_error(APLOG_MARK, APLOG_DEBUG, table_result, r->server,
			  "child attempted but failed to create one or more tables for %s, preserving query",
			  ap_get_server_name(r));
		preserve_entry(r, query);
		return LOGSQL_QUERY_PRESERVED;
	    } else {
		log_error(APLOG_MARK, APLOG_DEBUG, table_result, r->server, "tables successfully created - retrying query");

		result = global_config.driver->insert(r, &global_config.db, query);

		if (result != LOGSQL_QUERY_SUCCESS) {
		    log_error(APLOG_MARK, APLOG_DEBUG, result, r->server, "giving up, preserving query");
		    preserve_entry(r, query);
		    return LOGSQL_QUERY_PRESERVED;
		} else {
		    log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "query successful after table creation");
		    return LOGSQL_QUERY_SUCCESS;
		}
	    }
	}
	...
	return LOGSQL_QUERY_PRESERVED;
    default:
	log_error(APLOG_MARK, APLOG_DEBUG, 0, r->server, "Invalid return code from driver->insert");
...
static void *log_sql_merge_state(apr_pool_t * p, void *basev, void *addv)
{
    ...
    if (strcmp(child->preserve_file, DEFAULT_PRESERVE_FILE) == 0)
    ...
    if (strcmp(child->notes_table_name, DEFAULT_NOTES_TABLE_NAME) == 0)
	child->notes_table_name = parent->notes_table_name;

    if (strcmp(child->hin_table_name, DEFAULT_HIN_TABLE_NAME) == 0)
	child->hin_table_name = parent->hin_table_name;

    if (strcmp(child->hout_table_name, DEFAULT_HOUT_TABLE_NAME) == 0)
	child->hout_table_name = parent->hout_table_name;

    if (strcmp(child->cookie_table_name, DEFAULT_COOKIE_TABLE_NAME) == 0)
...
static int log_sql_transaction(request_rec * orig)
{
	...
	global_config.db.createtables = 1;
    }
	...
	ptrptr2 =
	    (char **) (cls->transfer_accept_list->elts +
		       (cls->transfer_accept_list->nelts * cls->transfer_accept_list->elt_size));
	...
	if ((r->uri) && (cls->transfer_accept_list->nelts)) {
	    proceed = 0;
	    for (ptrptr = (char **) cls->transfer_accept_list->elts; ptrptr < ptrptr2;
		 ptrptr = (char **) ((char *) ptrptr + cls->transfer_accept_list->elt_size))
	...
	ptrptr2 =
	    (char **) (cls->transfer_ignore_list->elts +
		       (cls->transfer_ignore_list->nelts * cls->transfer_ignore_list->elt_size));
	if (r->uri) {
	    for (ptrptr = (char **) cls->transfer_ignore_list->elts; ptrptr < ptrptr2;
		 ptrptr = (char **) ((char *) ptrptr + cls->transfer_ignore_list->elt_size))
		if (ap_strstr(r->uri, *ptrptr)) {
		    return OK;
		}
	}
	...
	ptrptr2 =
	    (char **) (cls->remhost_ignore_list->elts +
		       (cls->remhost_ignore_list->nelts * cls->remhost_ignore_list->elt_size));
	thehost = ap_get_remote_host(r->connection, r->per_dir_config, REMOTE_NAME, NULL);
	if (thehost) {
	    for (ptrptr = (char **) cls->remhost_ignore_list->elts; ptrptr < ptrptr2;
		 ptrptr = (char **) ((char *) ptrptr + cls->remhost_ignore_list->elt_size))
	...
	for (i = 0; i < cls->transfer_log_format->nelts; i++) {
		...
		log_error(APLOG_MARK, APLOG_DEBUG, 0, orig->server,
	    ...
	    formatted_item = item->func->func(item->func->want_orig_req ? orig : r, item->param ? item->param : NULL);
		...
		fields = apr_pstrcat(r->pool, fields, (showcomma ? "," : ""), item->sql_field_name, NULL);
		values = apr_pstrcat(r->pool, values, (showcomma ? "," : ""), global_config.driver->escape(r, formatted_item, r->pool, &global_config.db), NULL);
		showcomma = 1;
	    }
	}

	void build_itemsets(apr_array_header_t *whichheader,  apr_table_t *whichtable) {
	    ptrptr2 = (char **) (whichheader->elts + (whichheader->nelts * whichheader->elt_size));
	    for (ptrptr = (char **) whichheader->elts; ptrptr < ptrptr2;
		 ptrptr = (char **) ((char *) ptrptr + whichheader->elt_size)) {
		/* If the specified cookie/table (*ptrptr) exists for the current request... */
		if ((theitem = (whichtable ? apr_table_get(whichtable, *ptrptr) : extract_specific_cookie(r, *ptrptr) ))) {
		    str_trunc(*ptrptr, 80, 1);
		    str_trunc((char *) theitem, 255, 1);
	...
	build_itemsets(cls->notes_list, r->notes);
	if (*itemsets != '\0') {
	...
	build_itemsets(cls->hout_list, r->headers_out);
	if (*itemsets != '\0') {
	...
	build_itemsets(cls->hin_list, r->headers_in);
	if (*itemsets != '\0') {
	...
	build_itemsets(cls->cookie_list, NULL);
	if (*itemsets != '\0') {
	...
	/* How's our db link integrity? */
	if (!global_config.db.connected) {

	    /* Make a try to reestablish the link */
	    log_error(APLOG_MARK, APLOG_DEBUG, 0, orig->server, "mod_log_sql: trying to reestablish db link");
	    log_sql_opendb_link(orig->server);
		...
		log_error(APLOG_MARK, APLOG_DEBUG, 0, orig->server,
			  "mod_log_sql: child established database connection");
	    }
	}
...
static const command_rec log_sql_cmds[] = {
    ...
    AP_INIT_TAKE1("LogSQLLoginInfo", set_log_sql_info, NULL, RSRC_CONF,
    ...
    AP_INIT_FLAG("LogSQLCreateTables", set_global_nmv_flag_slot,
		 (void *) APR_OFFSETOF(global_config_t, db.createtables), RSRC_CONF,
    ...
    AP_INIT_RAW_ARGS("LogSQLRegisterItem", set_register_field,
		     ...
		     "datatypes are INT, SMALLINT, VARCHAR, CHAR, and TIMESTAMP<br>")
	,
