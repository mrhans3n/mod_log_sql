...
// From apache 2.2's mod_logio.c to provide logging ACTUAL incoming and outgoing bytes
...
static apr_status_t logio_out_filter(ap_filter_t * f, apr_bucket_brigade * bb)
{
	...
	APR_BUCKET_INSERT_BEFORE(b, apr_bucket_flush_create(f->c->bucket_alloc));
    }
...
static int post_config(apr_pool_t * p, apr_pool_t * plog, apr_pool_t * ptemp, server_rec * s)
{
    ...
    log_sql_register_field(p, "bytes_in", "bytes_in", NULL, "bytes_sent", LOGSQL_DATATYPE_INT, 20);
    log_sql_register_field(p, "bytes_out", "bytes_out", NULL, "bytes_recvd", LOGSQL_DATATYPE_INT, 20);
...
static void register_hooks(apr_pool_t * p)
{
    ...
    ap_register_input_filter(logio_filter_name, logio_in_filter, NULL, AP_FTYPE_NETWORK - 1);
    ap_register_output_filter(logio_filter_name, logio_out_filter, NULL, AP_FTYPE_NETWORK - 1);
